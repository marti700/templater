...
func TestDocumentPreview(t *testing.T) {
	...
	//
	reqRecorder := responseRecorderHelper(req, DocumentPreview("../tmpls/", "../preview.html"))
...
func TestUploadTemplate(t *testing.T) {
	...
	writer.Close()
	reqRecorder := httptest.NewRecorder()
	handler := http.HandlerFunc(Uploadtemplate("../tmpls/"))
	...
	uploadedFile, err := os.Open("../tmpls/testTemplate.txt")
